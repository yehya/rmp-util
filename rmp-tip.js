...
window.Rmptip = function (professorElement, prfData) {
	...
	var hotnessElement, profQuality, profAvgGrade, profHotness, profHelpfulness, profClarity, profEasiness, tipTitle, hotImg;
	var IMAGE_SRC, HOTNESS_IMAGES, HOTNESS;
    ...
    */
    ...
    var colorChooser = function(rating){
        ...
        */
        console.log(document.querySelector(".chili-background"));
        if (typeof rating === "number") {
	        switch (true) {
	        	case (rating > 3 && rating <= 5):
		            color = BAR_COLOR_STYLES[2];
		            hotImg = HOTNESS_IMAGES[3];
		            break;
	            case (rating > 1 && rating <= 3):
		            color = BAR_COLOR_STYLES[1];
		            hotImg = HOTNESS_IMAGES[2];
		            break;
		        default:
		            color = BAR_COLOR_STYLES[0];
		            hotImg = HOTNESS_IMAGES[1];
	        }
    	} else {
    		switch (rating) {
    			case ("A"):
